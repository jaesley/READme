module ApplicationHelper
  ...
  def get_author_single_page(page_number)
    reviews = RestClient.get "http://www.goodreads.com/review/list/#{@current_user.uid}?key=#{ENV['GOODREADS_API_KEY']}&sort=author&per_page=200&shelf=read"
    reviews = Hash.from_xml(reviews.to_s)
    total = reviews['GoodreadsResponse']['reviews']['total'].to_i
